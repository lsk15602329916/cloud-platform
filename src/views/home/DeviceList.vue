...
<script>
  ...
  export default {
    ...
    computed: {
      dialogComponent() {
        const dialogs = ['deviceListDialog', 'deviceDataListDialog']
        return dialogs[this.listIndex]
      },
      headers () {
        const headers = [ this.deviceHeaders, this.deviceDataHeaders]
        return headers[this.listIndex]
      },
      tableList() {
        const list = [this.deviceList, this.deviceDataList]
        return list[this.listIndex]
      }
    },
    async created () {
      try {
        await this.searchUserDevice()
      } catch (e) {
    ...
    methods: {
      ...
      async handlePageChange(page) {
        this.isLoading = true
        const methods = [this.getDeviceList, this.getDeviceData]
